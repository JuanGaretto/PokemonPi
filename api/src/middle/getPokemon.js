...
const getApiInfo = async () => {
    ...
    let arrayPoke = resu.map((result) => {  //DENTRO DE UN ARRAY ME TRAIGO TODAS LAS PROPIEDADES QUE QUIERO DE CADA POKEMON.
      return {
        id: result.id,
        name: result.name,
...
//CONCATENACION DE LOS DOS RESULTADOS ENCONTRADOS..
  const getAllPokemons = async () => { 
    ...
    const infoGeneral = apiInfo.concat(dbInfo); //CONCATENO LAS DOS Y RETORNO ESTO.
    return infoGeneral;
  };
